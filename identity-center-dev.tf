data "aws_identitystore_user" "dev_user" {
...
resource "aws_ssoadmin_permission_set" "dev" {
  name             = "PowerUserAccess-PS"
  description      = "Power user access for AWS users"
...
resource "aws_ssoadmin_account_assignment" "dev_assignment" {
  ...
  principal_type = "USER"
  principal_id   = data.aws_identitystore_user.dev_user.user_id
